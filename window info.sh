...
get_window_info() {
	...
	if [ "$notify" ]; then
		notify-send "Click on the window you want info from!" -u normal
	fi
...
window_info=`get_window_info | sort | tr "\n" " " | sed -r 's/^(.*) $/[\1]\n/'`
echo -n "$window_info" | xclip -selection c
if [ "$notify" ]; then
	notify-send "Copied to clipboard: $window_info" -u normal
fi
echo "Copied to clipboard: $window_info"
